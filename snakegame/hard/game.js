...
const best_score_hard_num = document.querySelector('.best-score-num');
...
let score;
let best_score_hard = 0;

if (localStorage.getItem("best_score_hard") !== null) {
    best_score_hard = (localStorage.getItem("best_score_hard"))
    if (best_score_hard > 9) {
        best_score_hard_num.textContent = best_score_hard;
    } else {
        best_score_hard_num.textContent = '0' + best_score_hard;
    }
}
...
function moveEverything() {
    ...
    if (snakePosX === foodPosX && snakePosY === foodPosY) {
        ...
        if (score > best_score_hard) {
            ...
            localStorage.setItem("best_score_hard", best_score_hard)
        }
    }
}
...
function warningHard() {
    canvas.classList.add('active');
    setTimeout(removeWarning, 1600);
}
